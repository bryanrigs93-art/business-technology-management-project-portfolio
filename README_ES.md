# Portafolio de Business Technology Management

Este repositorio es mi portafolio de proyectos prácticos relacionados con Business Technology Management, Excel, análisis de datos, documentación y Business Intelligence.

Mi objetivo es documentar proyectos pequeños, pero bien explicados, que muestren cómo trabajo con datos: revisar archivos, detectar problemas de calidad, organizar información, crear resúmenes, construir visualizaciones y explicar hallazgos de forma clara.

---

# Proyecto Excel: Limpieza y análisis descriptivo de datos de salud

Este es uno de mis primeros proyectos principales de Excel para portafolio. Trabajé con dos archivos académicos de Excel: uno enfocado en limpieza y calidad de datos, y otro enfocado en análisis descriptivo, tablas resumen y gráficos.

El proyecto está disponible en inglés y español.

## Estructura del proyecto

```text
projects/excel-patient-data-analysis/
├── english/
│   ├── 01_data_quality_cleaning_EN.xlsx
│   └── 02_patient_data_analysis_excel_EN.xlsx
└── spanish/
    ├── 01_limpieza_calidad_datos_ES.xlsx
    └── 02_analisis_pacientes_excel_ES.xlsx
```

---

## Presentación en Canva

https://canva.link/4t7szd1apthwldm

---

## Qué hice

El proyecto tiene dos partes.

### 1. Limpieza y control de calidad de datos

En el primer archivo revisé el dataset antes del análisis.

Trabajé en:

- Revisión de columnas, estructura y tipos de datos.
- Detección de valores faltantes.
- Revisión de registros duplicados.
- Identificación de valores inconsistentes o incompletos.
- Documentación de errores encontrados.
- Registro de correcciones aplicadas.
- Preparación de una versión más limpia de los datos.

### 2. Análisis descriptivo en Excel

En el segundo archivo practiqué análisis descriptivo.

Creé:

- Estadísticas básicas como conteo, promedio, mínimo y máximo.
- Tablas resumen.
- Gráficos para visualizar patrones.
- Una interpretación breve de los resultados.

---

## Aprendizajes principales

- La revisión de calidad de datos es necesaria antes de sacar conclusiones.
- Las hojas de errores y correcciones ayudan a documentar el proceso.
- Excel es útil para análisis inicial y reporting.
- Las tablas resumen ayudan a comparar categorías.
- Los gráficos deben apoyar la explicación, no solo decorar el archivo.

---

## Cómo explicaría este proyecto en una entrevista

Trabajé con dos archivos de Excel. Primero revisé la calidad de los datos buscando valores faltantes, duplicados, inconsistencias y errores lógicos. Después documenté los problemas encontrados y las correcciones aplicadas.

Luego utilicé otro archivo para realizar análisis descriptivo. Calculé estadísticas básicas, preparé tablas resumen, construí gráficos y redacté una explicación corta de los resultados.

Este proyecto muestra mi base inicial en Excel aplicado al análisis de datos y mi interés en avanzar hacia Business Intelligence.

---

## Herramientas utilizadas

- Microsoft Excel
- Filtros y ordenamiento
- Fórmulas básicas
- Tablas resumen
- Gráficos
- Canva
- GitHub

---

## Nota sobre los datos

Este proyecto se presenta con fines académicos y de portafolio. Los archivos se tratan como datos de práctica para aprender Excel, limpieza de datos y análisis descriptivo.

---

## Próximas mejoras

- Crear un dashboard más limpio en Excel.
- Rehacer el análisis en Power BI.
- Repetir la limpieza con Python y pandas.
- Agregar capturas del dashboard final.
- Crear un reporte PDF corto con conclusiones finales.
